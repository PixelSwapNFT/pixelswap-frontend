...
import { getPixelRabbitContract } from 'utils/contractHelpers'
import makeBatchRequest from 'utils/makeBatchRequest'

const pixelRabbitsContract = getPixelRabbitContract()
...
const useGetWalletNfts = () => {
  ...
  useEffect(() => {
    const fetchNfts = async () => {
      try {
        const balanceOf = await pixelRabbitsContract.methods.balanceOf(account).call()
        ...
        if (balanceOf > 0) {
          ...
          const getTokenIdAndBunnyId = async (index: number) => {
            try {
              const { tokenOfOwnerByIndex, getBunnyId, tokenURI } = pixelRabbitsContract.methods
