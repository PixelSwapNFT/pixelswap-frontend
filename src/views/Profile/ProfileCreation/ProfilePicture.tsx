...
import { AutoRenewIcon, Button, Card, CardBody, Heading, Skeleton, Text } from 'pixelswap-uikit'
import { Link as RouterLink } from 'react-router-dom'
import { useWeb3React } from '@web3-react/core'
import nftList from 'config/constants/nfts'
import useI18n from 'hooks/useI18n'
import { useToast } from 'state/hooks'
import { getPixelProfileAddress } from 'utils/addressHelpers'
...
const ProfilePicture: React.FC = () => {
  ...
  const handleApprove = () => {
    pixelRabbitsContract.methods
      .approve(getPixelProfileAddress(), tokenId)
      .send({ from: account })
      .on('sending', () => {
        setIsApproving(true)
      })
    ...
    return (
      <>
        ...
        <Text bold fontSize="20px" mb="24px">
          {TranslateString(854, 'We couldn’t find any Pixel Collectibles in your wallet.')}
        </Text>
        <Text as="p">
          {TranslateString(
            856,
            'You need a Pixel Collectible to finish setting up your profile. If you sold or transferred your starter collectible to another wallet, you’ll need to get it back or acquire a new one somehow. You can’t make a new starter with this wallet address.',
          )}
        </Text>
      </>
    )
      ...
      <Card mb="24px">
        <CardBody>
          ...
          <Text as="p" color="textSubtle" mb="24px">
            {TranslateString(816, 'Only approved Pixel Collectibles can be used.')}
